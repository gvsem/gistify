...
export module Gistify {
    ...
    export class Publish {
        ...
        public static toPastebin(document: vscode.TextDocument, preferSelection: boolean, anonymously: boolean, privacy: Pastebin.Privacy, expire: Pastebin.ExpireDate) {
            ...
            new Promise(() => {
                ...
                let clientPromise: Promise<Pastebin.Client>;
    ...
    export function selectService(): Thenable<number> {
        ...
        return new Promise<number>((accept, reject) => {
            vscode.window.showQuickPick(
                services, { title: "Select service to publish:" }
            ).then((selection: IterableQuickPick | undefined) => {
    ...
    export function selectSource(): Thenable<boolean> {
        let fileOrSelection: Array<IterableQuickPick> = [
            {
                ...
                description: "Publish current opened file",
                i: 0
            },
            {
        ...
        return new Promise<boolean>((accept, reject) => {
            let selectionPromise = vscode.window.showQuickPick(fileOrSelection, { title: "Select snippet source:" });
    ...
    export function publishPastebin(d: vscode.TextDocument, wholeFile: boolean, anonymous: boolean): Thenable<void> {
        ...
        return vscode.window.showQuickPick(
            pasteBinPrivacy, { title: "Specify Pastebin privacy:" }
        ).then((selection: IterableQuickPick | undefined) => {
            ...
            vscode.window.showQuickPick(
                pasteBinExpire, { title: "Specify Pastebin expire date:" }
            ).then((selection: vscode.QuickPickItem | undefined) => {

                if (selection !== undefined) {
                    let exp = selection.label as Pastebin.ExpireDate;
    ...
    export function publishGists(d: vscode.TextDocument, wholeFile: boolean): Thenable<void> {
        ...
        return vscode.window.showQuickPick(
            gistsPrivacy, { title: "Specify Gists privacy:" }
        ).then((selection: IterableQuickPick | undefined) => {
            ...
            vscode.window.showInputBox(
                { title: "Optionally: specify description for this gist." }
            ).then((description: string | undefined) => {

                if (description !== undefined) {
                    Gistify.Publish.toGists(
                        d!, !wholeFile, (privacy === 0 ? Gists.Privacy.public : Gists.Privacy.private), description!
                    );
                }
